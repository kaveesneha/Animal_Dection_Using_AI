...
#ret, frame = cap.read()
frame = cv2.imread("static/dataset/r2.jpg")
...
for i in range(detections.shape[2]):
    ...
    if confidence > args.thr: # Filter prediction 
        ...
        try:
            ...
            x=xLeftBottom
            w=xRightTop-x
            image = cv2.imread("static/dataset/r2.jpg")
            mm=cv2.rectangle(image, (x, y), (x+w, y+h), (0, 255, 0), 2)
            cv2.imwrite("static/dataset/result.jpg", mm)
            cropped = image[yLeftBottom:yRightTop, xLeftBottom:xRightTop]
            gg="segment.jpg"
            cv2.imwrite("static/dataset/"+gg, cropped)
